...
export class TimeBoundary extends PureComponent<QueryBuilderComponentProps, State> {
  ...
  selectOptions: Record<string, Array<SelectableValue<string>>> = {
    bound: [
      ...
      { label: 'Min time', value: 'minTime' },
    ],
  };

  constructor(props: QueryBuilderProps) {
    super(props);
    this.resetBuilder(['queryType', 'dataSource', 'bound', 'filter', 'intervals']);
    const { builder } = props.options;
    builder.queryType = 'timeBoundary';
      ...
      <>
        <div className="gf-form">
          ...
          >
            ...
            />
            <label className="gf-form-label">Bound</label>
            <Select
              options={this.selectOptions.bound}
              value={this.selectOptionByValue('bound', builder.bound)}
              allowCustomValue
